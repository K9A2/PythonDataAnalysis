...
def main():
    ...
    doc = re.sub("[\n|\t]", "", "".join(JSONDecoder.read_text_file(file_name)))
    json_object = json.loads("".join(doc))

    intervals = json_object["intervals"]

    socket_keys = JSONDecoder.get_socket_keys(intervals)

    result = JSONDecoder.get_result_dictionary(intervals, socket_keys)
